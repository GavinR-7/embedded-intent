  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line bg-void/85 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-content items-center justify-between gap-4 px-gutter">
        <Link
          href="/"
          className="flex items-center gap-2.5 rounded-sm text-ink transition-colors duration-[var(--duration-fast)] hover:text-signal"
          onClick={() => closeMenu(false)}
        >
          <ChipMark className="h-6 w-6 text-signal" />
      ...
      */}
      {open && (
        <div
          id="mobile-menu"
          ref={panelRef}
          className="fixed inset-x-0 top-16 bottom-0 z-40 overflow-y-auto border-t border-line bg-void md:hidden motion-safe:animate-[menu-in_var(--duration-base)_var(--ease-out-expo)]"
        >
